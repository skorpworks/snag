...
my $monitored_procs = {
                        ...
                        regexps => [ qr/_SNAGs.pl$/, qr/_SNAGp.pl$/, ]
                      };
    ...
    POE::Session->create(
        inline_states => {
            ...
            query_alert_state => sub {
                ...
                my $sql =
"SELECT aid, host, source, category, alert, count, first_seen, last_seen from alerts where last_seen > (SELECT first_seen from alerts where first_seen IS NOT NULL order by last_seen desc limit 1)  - interval '2 hour' order by last_seen desc";
                print "Executing: $sql\n" if $debug;
...
"DEBUG: Alerts: alert_state: setting state for aid $row->{aid} to fs:$fs ls:$ls c:$row->{count}"
                              )
                              if $debug;
                        };
                        if ($@) {
            ...
            alert_expire => sub {
                ...
                while ( ( $key, $value ) = each %$a_ids ) {
                    ...
                    if ( $value >= ( 60 * 60 ) ) {
                        ...
                        $kernel->call( 'logger' => 'log' =>
"DEBUGGING: aid: $key  hseen: $heap->{a_ids}->{seen}->{$key}  fseen: $state->{alerts}->{$alert}->{first_seen}  lseen: $state->{alerts}->{$alert}->{last_seen} alert: $alert\n"
                          )
                          if $debug;
            ...
            alerts_settings => sub {
                ...
                if ( $dbires->{error} ) {
                    ...
                    foreach $row ( @{ $dbires->{result} } ) {
                        ...
                        $kernel->call( 'logger' => 'log' =>
"NOTIFY: alert_settings ($alert_string) with sev:$row->{'severity'} and email: $row->{'email'}\n"
                          )
                          if $debug;
                        eval {
            ...
            notify => sub {
                ...
                return
                  unless ( defined $state->{'alert_settings'}->{'email'}->{"$alert_string"}
                           && $state->{'alert_settings'}->{'email'}->{"$alert_string"} ne '' );
                ...
                my %mail = (
                             ...
                             Subject => "SNAG "
                               . $state->{'alert_settings'}->{'sev'}->{"$alert_string"}
                               . " alert from "
                               . $host_name . "!",
                ...
                $html .=
                    "<tr><td>Processed &nbsp;&nbsp;</td><td>: "
                  . encode_entities( time2str( "%Y-%m-%d %X", time() ) )
                  . "</td></tr>\n";
                $html .= "</table>\n";
...
END_OF_BODY
                ...
                unless ( $heap->{notify_wheel} ) {
                    ...
                    $kernel->yield( 'log' =>
"Could not send alert because an alert wheel is already running.  Subject: $mail{Subject}, Message: $mail{Message}"
                    );
                }
            },
            ...
            notify_stderr => sub {
                my ( $kernel, $heap, $error ) = @_[ KERNEL, HEAP, ARG0 ];
                $kernel->call( 'logger' => 'log' =>
"Could not send alert because of an error.  Error: $error, Subject: $heap->{mail_args}->{Subject}, Message: $heap->{mail_args}->{Message}"
                );
            },
        ...
        foreach $row (@$parcel) {
            ...
            if ( $row =~ m/^heartbeat_syslog/ ) {
                ...
                unless ( $p_seen =~ m/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/ ) {
                    $kernel->call( 'logger' => 'log' =>
                          "heartbeat_syslog: $p_host.$p_fqdn.$p_loghost: ERRROR: improper format for timestamp $p_seen "
                      )
                      if $debug;
                    next;
                }
                unless ( defined $heap->{hbs_s_sth} ) {
                    $heap->{hbs_s_sth} = $heap->{dbh}
                      ->prepare('SELECT count(*) from heartbeat_syslog where host= ? and fqdn = ? and loghost= ?');
                    $heap->{hbs_i_sth} = $heap->{dbh}->prepare('INSERT INTO heartbeat_syslog values (?, ?, ?, ?)');
                    $heap->{hbs_u_sth} = $heap->{dbh}
                      ->prepare('UPDATE heartbeat_syslog set seen = ? where host= ? and fqdn = ? and loghost= ?');
                    $kernel->call( 'logger' => 'log' => "DEBUG: Alerts: preparing hbs_*_sth" ) if $debug;
                }
                unless ( defined $heap->{hbs_ids}->{"$p_host.$p_fqdn.$p_loghost"} ) {
                    $kernel->call(
                              'logger' => 'log' => "DEBUG: Alerts: inserting to hbs_i_sth: $p_host.$p_fqdn.$p_loghost" )
                      if $debug;
                    $heap->{hbs_s_sth}->execute( $p_host, $p_fqdn, $p_loghost )
                      or die "hbs_s_sth failed: " . $heap->{dbh}->errstr;
                    my $ary_ref = $heap->{hbs_s_sth}->fetchall_arrayref() or die "hbs_s_sth failed: ";
                    if ( defined $ary_ref && ${$ary_ref}[0][0] > 0 ) {
                        $kernel->call(
                               'logger' => 'log' => "DEBUG: Alerts: updating to hbs_u_sth: $p_host.$p_fqdn.$p_loghost" )
                          if $debug;
                        $heap->{hbs_u_sth}->execute( $p_seen, $p_host, $p_fqdn, $p_loghost )
                          or die "hbs_u_sth 1 failed: " . $heap->{dbh}->errstr;
                    }
                    else {
                        $kernel->call(
                              'logger' => 'log' => "DEBUG: Alerts: inserting to hbs_i_sth: $p_host.$p_fqdn.$p_loghost" )
                          if $debug;
                        $heap->{hbs_i_sth}->execute( $p_host, $p_fqdn, $p_loghost, $p_seen )
                          or die "hbs_i_sth failed: " . $heap->{dbh}->errstr;
                    }
                    ...
                    $kernel->call(
                      ...
                      if $debug;
                    $heap->{hbs_u_sth}->execute( $p_seen, $p_host, $p_fqdn, $p_loghost )
                      or die "hbs_u_sth 2 failed: " . $heap->{dbh}->errstr;
                }
            }
                ...
                unless ( defined $heap->{create_alrt_sth} ) {
                    $heap->{create_alrt_sth} =
                      $heap->{dbh}->prepare(
"INSERT INTO alerts (host, source, category, alert, count, first_seen, last_seen) values (?, ?, ?, ?, '1', ?, ?)" );
                    $kernel->call( 'logger' => 'log' => "DEBUG: Alerts: preparing create_alrt_sth" ) if $debug;
                }
                unless ( defined $heap->{update_alrt_sth} ) {
                    $heap->{update_alrt_sth} =
                      $heap->{dbh}->prepare('UPDATE alerts set count = ?, last_seen = ? where aid = ?');
                    $kernel->call( 'logger' => 'log' => "DEBUG: Alerts: preparing update_alrt_sth" ) if $debug;
                }

                my ( $p_table, $p_host, $p_source, $p_category, $p_alert, $p_full, $p_param, $p_seen ) =
                  split /$rec_sep/, $row, -1;
                ...
                unless ( defined $state->{alerts}->{"$alert_string"}->{first_seen}
                         && ( $curr_seen - $state->{alerts}->{"$alert_string"}->{last_seen} < 660 ) )
                {
#$kernel->post('notifier' => 'notify' => { 'host' => $p_host, 'source' => $p_source, 'category' => $p_category, 'alert' => $p_alert, 'timestamp' => $curr_seen } );
                    ...
                    $heap->{create_alrt_sth}->execute( $p_host, $p_source, $p_category, $p_alert, $p_seen, $p_seen )
                      or die "create_alrt_sth failed: " . $heap->{dbh}->errstr;
                    my $aid = $heap->{dbh}->last_insert_id( undef, undef, 'alerts', undef );

                    $kernel->call( 'logger' => 'log' =>
"DEBUG: Alerts: creating new alert $aid for #$alert_string# #cs:$curr_seen# #fs:$state->{alerts}->{$alert_string}->{first_seen}# #ls:$state->{alerts}->{$alert_string}->{last_seen}# #aid:$heap->{a_ids}->{seen}->{$state->{alerts}->{$alert_string}->{aid}}#"
                    );
                    $state->{alerts}->{"$alert_string"}->{count}                            = 1;
                    $state->{alerts}->{"$alert_string"}->{first_seen}                       = $curr_seen;
                    $state->{alerts}->{"$alert_string"}->{last_seen}                        = $curr_seen;
...
#          $kernel->call('logger' => 'log' => "DEBUG: Alerts: reusing alert $state->{alerts}->{$alert_string}->{aid} for #$alert_string#  #$state->{alerts}->{$alert_string}->{first_seen}# $state->{alerts}->{$alert_string}->{last_seen}# #$heap->{a_ids}->{seen}->{$state->{alerts}->{$alert_string}->{aid}}#: $p_seen");
                    ...
                    $heap->{a_ids}->{seen}->{ $state->{alerts}->{"$alert_string"}->{aid} }  = 0;
                    $heap->{a_ids}->{alert}->{ $state->{alerts}->{"$alert_string"}->{aid} } = $alert_string
                      unless defined $heap->{a_ids}->{alert}->{ $state->{alerts}->{"$alert_string"}->{aid} };
                    $heap->{update_alrt_sth}->execute( $state->{alerts}->{"$alert_string"}->{count},
                                                       $p_seen, $state->{alerts}->{"$alert_string"}->{aid} )
                      or die "update_alrt_sth failed: " . $heap->{dbh}->errstr;
                }

                #$heap->{dbh}->commit();
                #$heap->{dbh}->begin_work();
                $heap->{evt_sth}->execute( $p_full, $p_param, $p_seen, $state->{alerts}->{"$alert_string"}->{aid} )
                  or die "Failed loading: event: \'$row\' :" . $heap->{dbh}->errstr;
                $heap->{dbh}->commit();
