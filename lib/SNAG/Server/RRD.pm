...
################################
{
    ...
    croak "Args must contain values for 'dir', 'dsn', 'user', and 'pw'"
      unless ( $args->{dir} && $args->{dsn} && $args->{user} && $args->{pw} );
        ...
        inline_states => {
            ...
            conn_handler => sub {
                ...
                if ( defined( $heap->{disconnect_time} ) ) {
                    $kernel->call( 'logger' => "log" => "EASYDBI: Connected to db - was down for "
                                   . ( time() - $heap->{disconnect_time} ) );
        ...
        inline_states => {
            ...
            build_threshholds => sub {
                ...
                if ($@) {
                    $kernel->call( 'logger' => "log" =>
                                   "DASHBOARD: build_threshholds: failed loading $base_dir/RULES.xml : $@\n" );
                }
                else {
                    ...
                    my (@xml)       = split /\n/, $xmlstring;
                    foreach my $line (@xml) {
                        $kernel->call( 'logger' => "log" => "DASHBOARD: build_threshholds: $line" );
                    }
              ...
              TOP: for ( $i = 0 ; $i <= 10 ; $i++ ) {
                    ...
                    while ( ( $ds, $obj ) = each %$val ) {
                        ...
                        if ($@) { delete $rrd->{$host_multi}->{$ds}; }    ###THE RRD MUST HAVE BEEN DELETED
                            ...
                            eval { $kernel->post( 'dbi_helper' => 'update_query' => $host, $multi, $ds, $seen ); }
                        }
                        $stats->{easydbi}->{scan}++;
                    }    ### end while loop
                    shift @scan_queue;
            ...
            load => sub {
                ...
                foreach my $row (@$parcel) {
                    ...
                    if ($@) {
                        if (    $@ =~ /Can\'t call method \"update\" on an undefined value/
                             || $@ =~ /No such file or directory/ )
                        {
                            ...
                            unless ( $hostkey && $ds && $type && $time && defined $value ) {
                                ...
                                $kernel->call(
                                             'logger' => "log" => "LOAD: Bogus line: $row, invalid hostkey: $hostkey" );
                            }
                            elsif ( $host !~ /^([\w\-\.])+$/ ) {
                                ...
                                unless ( -d $rrd_dir ) {
                                    ...
                                    unless ( -d $dir ) {
                                        mkdir $dir, 0750
                                          and $kernel->call( 'logger' => "log" => "LOAD: Creating $dir" );
                                    }
                                    mkdir $rrd_dir, 0750
...
#	$kernel->call('logger' => "log" => "rrd migrate: host=$host result=$result elapsed=$elapsed close_status=$close_status exit_status=$exit_status rsync_output=$rsync_output source_host=$source_host");
                                ...
                                eval {
                                    ...
                                    unless ( -e $rrd_file ) {
                                        ...
                                        $rrd->{$hostkey}->{$ds}
                                          ->create( @{ &get_template( $ds, $n, $t, $a, $i, $time ) } );
                                        chmod 0740, $rrd_file
                                          || $kernel->call( 'logger' => "log" => "Error chmoding: $rrd_file" );
...
#$kernel->post('client' => 'dashboard' => 'load' => join $rec_sep, ('events', $host, 'RRD', 'rrd error', 'RRD create error', $@,'',time2str("%Y-%m-%d %T", $time)));
                                ...
                                $kernel->post(
                                               'client' => 'dashboard' => 'load' => join $rec_sep,
                                               (
                                                 'events', $host, 'RRD', 'rrd error', 'RRD create error',
                                                 $@, '', time2str( "%Y-%m-%d %T", $time )
                                               )
                                             );
                        ...
                        if ( $heap->{threshholds}->{$ds} ) {
                            ...
                            if ( $type =~ /(\d+)[c|d]/i ) {
                                eval {
                                    $rrd->{$hostkey}->{$ds}
                                      ->fetch_start( cfunc => "LAST", start => ( $time - 10 ), end => ( $time - 1 ) );
                            ...
                            foreach $threshhold ( @{ $heap->{threshholds}->{$ds} } ) {
                                ...
                                if (    defined $threshhold->{agg}
                                     && defined $heap->{dashboard_store}->{$host}->{ $threshhold->{agg} } )
                                {
                                ...
                                $dbevent->{full}    = '';
                                $kernel->call( 'logger' => "log" =>
"DASHBOARD: $hostkey:$ds ($threshhold->{'type'}:$threshhold->{'limit'}) $dbevent->{'value'}"
                                  )
                                  if $debug;
                                if (    $threshhold->{type} eq 'range'
                                     && $dbevent->{'value'} <= $threshhold->{high}
                                     && $dbevent->{'value'} >= $threshhold->{low} )
                                {
                                    $dbevent->{full} =
"$threshhold->{alert} ($threshhold->{low} < $dbevent->{'value'} <  $threshhold->{high})";
                                }
                                if ( $threshhold->{type} eq 'ceiling' && $dbevent->{'value'} >= $threshhold->{limit} ) {
                                    $dbevent->{full} =
                                      "$threshhold->{alert} ($dbevent->{'value'} > $threshhold->{limit})";
                                }
                                elsif ( $threshhold->{type} eq 'floor' && $dbevent->{'value'} <= $threshhold->{limit} )
                                {
                                    ...
                                    if ( defined $heap->{dashboard_store}->{$host}->{$ds}
                                         && $dbevent->{'value'} < $heap->{dashboard_store}->{$host}->{$ds} )
                                    {
                                        $dbevent->{full} =
"$threshhold->{alert} ($dbevent->{'value'} < $heap->{dashboard_store}->{$host}->{$ds})";
                                    }
                                    ...
                                    if ( defined $heap->{dashboard_store}->{$host}->{ $threshhold->{denom} } ) {
                                        unless (    $dbevent->{'value'} <= 0
                                                 || $heap->{dashboard_store}->{$host}->{ $threshhold->{denom} } <= 0 )
                                        {
                                            $dbevent->{full} =
                                              "$threshhold->{alert} ($dbevent->{'value'} > $threshhold->{limit})"
                                              if (
                                                   (
                                                     $dbevent->{'value'} /
                                                     $heap->{dashboard_store}->{$host}->{ $threshhold->{denom} }
                                                   ) <= $threshhold->{ratio}
                                                 );
                                        }
                                    }
                                ...
                                if ( $dbevent->{full} && $dbevent->{full} ne '' ) {
                                    ...
                                    $dbevent->{message} = join $rec_sep,
                                      (
                                        'events', $host, 'sysrrd', $threshhold->{type}, $threshhold->{alert},
                                        $dbevent->{full}, $dbevent->{param}, $dbevent->{'time'}
                                      );
