        ...
        inline_states => {
            ...
            sysinfo_query => sub {
                ...
                if ( $heap->{dbh} && $heap->{connected} ) {
                             ...
                             || $@ =~ /server closed the connection unexpectedly/ )
                        {
                            $kernel->call(
                                           'logger' => 'alert' => {
                                               ...
                                               Subject => "Error on " . HOST_NAME . "::sysinfo::sysinfo_query::$sender",
                                               Message => $@
                                           }
                                         );
                    ...
                    $kernel->call(
                                  'logger' => 'log' => "Could not run $sender via $_[STATE], not connected to server" );
                }
            },
            ...
            build_monitor_defs => sub {
                ...
                my $coderef = sub {
                    ...
                    my $get_monitor_defs =
                      $heap->{dbh}->selectall_arrayref( 'select * from monitor_defs', { Slice => {} } );
            ...
            build_xen_uuids => sub {
                ...
                my $coderef = sub {
                    ...
                    my $get_uuids =
                      $heap->{dbh}
                      ->selectall_arrayref( 'select lower(uuid) as uuid,host from device where vendor = \'Xen\'',
                                            { Slice => {} } );
            ...
            build_remote_hosts => sub {
                ...
                my $coderef = sub {
                    ...
                    my $get_ips =
                      $heap->{dbh}->selectall_arrayref(
"select iface.host as host, iface.ip as ip from iface, heartbeat where iface.host = heartbeat.host and heartbeat.activity = 'active'",
                        { Slice => {} }
                      );
                    ...
                    my $get_ports =
                      $heap->{dbh}->selectall_arrayref(
"select distinct(port) as port from heartbeat, listening_ports where listening_ports.host = heartbeat.host and heartbeat.activity = 'active'",
                        { Slice => {} }
                      );
            ...
            host_passwd_login => sub {
                ...
                my $coderef = sub {
                    ...
                    $heap->{sth}->{passwd_login}->execute( $input->{seen}, $input->{dns}, $input->{uid} )
                      or die $heap->{dbh}->errstr;

                    $kernel->call(
                           'logger' => 'log' => "host_passwd_login: $input->{uid} to $input->{dns} at $input->{seen}" );
                    ...
                    foreach my $row (@$parcel) {
                        ...
                        unless ($no_heartbeat) {
                            ...
                            unless ( $heap->{sth}->{hb_update}->execute( $seen, $host, $alias, $source ) == 1 ) {
                                $heap->{sth}->{hb_insert}->execute( $host, $alias, $source, $seen )
                                  or die "Unable to update heartbeat: " . $heap->{sth}->errstr;

                                $kernel->post(
                                               'client' => 'dashboard' => 'load' => join $rec_sep,
                                               (
                                                 'events', $host, 'sysinfo', 'new_host',
                                                 'new host reported in!',
                                                 'new host reported in!',
                                                 '', $seen
                                               )
                                             );
                        ...
                        $kernel->call(   'logger' => 'log' => "Processing host:$host tables:"
                                       . int( keys %{$system_info} )
                                       . " seen:$heap->{seen}" );
                        ...
                        foreach my $table ( keys %{$system_info} ) {
                            ...
                            if ( ref $instructions->{$table}->{ignore} ne 'ARRAY'
                                 && $instructions->{$table}->{ignore} == 1 )
                            {
                            ...
                            if ( $table eq 'tags' ) {
                                ...
                                if (@$new) {
                                    $heap->{sth}->{tags_delete}->execute( $host, $cat )
                                      or die $heap->{dbh}->errstr . ": Could not delete $host data from tags";

                                    foreach my $ref (@$new) {
                                        $heap->{sth}->{tags_insert}
                                          ->execute( $host, $seen, $ref->{tag}, $cat, ( $ref->{multi} || '' ) )
                                          or die ": Could not update tags table on host '$host': "
                                          . $heap->{dbh}->errstr . "\n"
                                          . Dumper $new;
                                    }
                                }
                                ...
                                foreach my $conf ( keys %{ $system_info->{$table} } ) {
                                    ...
                                    my $fetch_old = $heap->{dbh}->selectrow_hashref(
                                                      "select * from $table where host = '$host' and source = '$conf'");
                                    if ($fetch_old) {
                                    ...
                                    unless ( $old_contents eq $new_contents ) {
                                        $kernel->call(
                                                      'logger' => 'log' => "Processing host:$host. conf:$conf changed" )
                                          if $debug;

                                        $heap->{sth}->{$table}->{'delete'}->execute( $host, $conf )
                                          or die $heap->{dbh}->errstr;
                                        ...
                                        $heap->{sth}->{$table}->{insert}
                                          ->execute( map { $insert_args->{$_} } @{ $heap->{cols}->{$table} } )
                                          or die ": Could not insert new data on host '$host', table '$table': "
                                          . $heap->{dbh}->errstr;
                                        ...
                                        unless ( $instructions->{$table}->{$conf}->{nohistory} ) {
                                            ...
                                            $heap->{sth}->{archive}
                                              ->execute( $host, $table, $conf, $new_contents, $diff, $seen )
                                              or die $heap->{dbh}->errstr;

                                            unless ( $instructions->{$table}->{$conf}->{nodashboard} ) {
                                                my $new_hid = $heap->{dbh}->last_insert_id( undef, undef, undef, undef,
                                                                                     { sequence => 'hist_index_seq' } );
                                                $kernel->post(
                                                               'client' => 'dashboard' => 'load' => join $rec_sep,
                                                               (
                                                                 'events', $host, 'sysinfo', 'config_change',
                                                                 'sysinfo configuration change',
                                                                 "$conf was changed",
                                                                 $new_hid, $seen
                                                               )
                                                             );
                                        ...
                                        if ( $conf eq '/etc/passwd' ) {
                                            ...
                                            foreach my $uid ( keys %{ $passwd->{old_contents} } ) {
                                                unless ( $passwd->{new_contents}->{$uid} ) {
                                                    $kernel->call(
                                                          'logger' => 'log' => "host_passwd: removed $uid from $host" );
                                                    $heap->{sth}->{passwd_remove}->execute( $seen, $host, $uid )
                                                      or die $heap->{dbh}->errstr;
                                                }
                                            }
                                            ...
                                            foreach my $uid ( keys %{ $passwd->{new_contents} } ) {
                                                unless (
                                                         cmpStr(
                                                                 $passwd->{old_contents}->{$uid},
                                                                 $passwd->{new_contents}->{$uid}
                                                         ) == 0
                                                       )
                                                {
                                                    ...
                                                    if ( defined $passwd->{old_contents}->{$uid} ) {
                                                        $kernel->call(
                                                            'logger' => 'log' => "host_passwd: changed $uid on $host" );
                                                        $heap->{sth}->{passwd_change}
                                                          ->execute( @args[ 0 .. 5 ], $host, $uid )
                                                          or die $heap->{dbh}->errstr;
                                                    }
                                                    else {
                                                        $kernel->call(
                                                              'logger' => 'log' => "host_passwd: added $uid to $host" );
                                                        $heap->{sth}->{passwd_add}
                                                          ->execute( $host, $uid, @args[ 0 .. 5 ], $seen )
                                                          or die $heap->{dbh}->errstr;
                                                    }
                                                }
                                            }
                                        ...
                                        $kernel->call(
                                                    'logger' => 'log' => "Processing host:$host. conf:$conf no change" )
                                          if $debug;
                                    }
                                }
                            }
...
"Processing host:$host. ignore key for $table needs to be an arrayref"
                                                );
                                            }
                                        }
                                    }
                                ...
                                my $fetch_old = $heap->{dbh}
                                  ->selectall_arrayref( "select * from $table where host = '$host'", { Slice => {} } );
                                if ($fetch_old) {
                                ...
                                unless ( cmpStr( $old_data, $new_data ) == 0 ) {
                                    ...
                                    if ( my $diff = diff \$old_data_flat, \$new_data_flat, { CONTEXT => 0 } ) {
                                        ...
                                        unless ( $instructions->{$table}->{nohistory} ) {
                                            $heap->{sth}->{archive}
                                              ->execute( $host, $table, '', $new_data_flat, $diff, $seen )
                                              or die $heap->{dbh}->errstr;
                                        ...
                                        $kernel->call(
                                                    'logger' => 'log' => "Processing host:$host. no change (check 2)" );
                                    }
                                }
                                else {
                ...
                if ($@) {
                         ...
                         || $@ =~ /server closed the connection unexpectedly/ )
                    {
                        $kernel->call(
                                       'logger' => 'alert' => {
                                                                From    => 'SNAGcnb@asu.edu',
                                                                To      => 'sporkworks@asu.edu',
                                                                Subject => "Error on " . HOST_NAME . "::sysinfo",
                                                                Message => $@
                                                              }
                                     );
                        $kernel->call( 'logger' => 'log' => $@ );
                        ...
                        $kernel->post(
                                       'client' => 'dashboard' => 'load' => join $rec_sep,
                                       (
                                         'events',        $heap->{host}, 'sysinfo', 'database error',
                                         'DB load error', $@,            '',        $heap->{seen}
                                       )
                                     );
            ...
            connect => sub {
                ...
                eval {
                    my $dbh;
                    $dbh =
                      DBI->connect( $db_args->{dsn}, $db_args->{user}, $db_args->{pw},
                    ...
                    foreach my $table (@$tables) {
                        ...
                        my $insert_sql =
                            "insert into $table ("
                          . ( join ", ", @{ $heap->{cols}->{$table} } ) . ") "
                          . "values ("
                          . ( join ", ", map { '?' } map { s/\"//g } @{ $heap->{cols}->{$table} } ) . ")";

                        $heap->{sth}->{$table}->{insert} = $heap->{dbh}->prepare($insert_sql);

                        if ( $table eq 'conf' ) {
                            $heap->{sth}->{$table}->{'delete'} =
                              $heap->{dbh}->prepare("delete from $table where host = ? and source = ?");
                        }
                        else {
                            $heap->{sth}->{$table}->{'delete'} =
                              $heap->{dbh}->prepare("delete from $table where host = ?");
                        }
                    }

                    $heap->{sth}->{hb_insert} =
                      $heap->{dbh}->prepare(
"insert into server_heartbeats(host, server, source, seen, server_seen) values(?, ?, ?, ?, now() )" );
                    $heap->{sth}->{hb_update} =
                      $heap->{dbh}->prepare(
"update server_heartbeats set seen = ?, server_seen = now() where host = ? and server = ? and source = ?" );

                    $heap->{sth}->{archive} = $heap->{dbh}
                      ->prepare("insert into hist(host, tab, source, data, diff, seen) values(?, ?, ?, ?, ?, ?)");

                    $heap->{sth}->{tags_delete} =
                      $heap->{dbh}->prepare("delete from tags where host = ? and category = ?");
                    $heap->{sth}->{tags_insert} = $heap->{dbh}
                      ->prepare("insert into tags (host, seen, tag, category, multi) values (?, ?, ?, ?, ?)");

                    $heap->{sth}->{passwd_add} =
                      $heap->{dbh}->prepare(
"insert into host_passwd(host, name, passwd, uid, gid, gecos, dir, shell, date_added) values(?, ?, ?, ?, ?, ?, ?, ?, ?)"
                      );
                    $heap->{sth}->{passwd_remove} =
                      $heap->{dbh}->prepare("update host_passwd set date_removed = ? where host = ? and name = ?");
                    $heap->{sth}->{passwd_change} =
                      $heap->{dbh}->prepare(
"update host_passwd set passwd = ?, uid = ?, gid = ?, gecos = ?, dir = ?, shell = ? where host = ? and name = ?" );
                    $heap->{sth}->{passwd_login} =
                      $heap->{dbh}->prepare(
"update host_passwd set last_login = ? where host = (select heartbeat.host from dns_aliases, heartbeat where heartbeat.host = dns_aliases.host and alias = ? order by heartbeat.seen desc limit 1) and name = ?"
                      );
                };
                if ($@) {
