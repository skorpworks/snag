package SNAG::Source::SystemStats::Linux::RHEL4;
...
sub run_netstat
{
  ...
  (
    Program      => [ "/opt/local/SNAG/bin/netstat -anT --tcp" ],
...
=cut
  ...
  if ($output =~ /^(\w+)\s+\d+\.\d+\s+/ && $heap->{iostat_io_count}->{$1}++ >= 1)
  {
    ...
    $kernel->post('client' => 'sysrrd' => 'load' => join $del, ("$host\[$mp\]", 'iorkbs', "1g", $time, $stats[7]));
    $kernel->post('client' => 'sysrrd' => 'load' => join $del, ("$host\[$mp\]", 'iowkbs', "1g", $time, $stats[8]));
    $kernel->post('client' => 'sysrrd' => 'load' => join $del, ("$host\[$mp\]", 'avgrqsz', "1g", $time, $stats[9]));
    $kernel->post('client' => 'sysrrd' => 'load' => join $del, ("$host\[$mp\]", 'avgqusz', "1g", $time, $stats[10]));
    $kernel->post('client' => 'sysrrd' => 'load' => join $del, ("$host\[$mp\]", 'await', "1g", $time, $stats[11]));
    $kernel->post('client' => 'sysrrd' => 'load' => join $del, ("$host\[$mp\]", 'svctm', "1g", $time, $stats[12]));
  }
}
