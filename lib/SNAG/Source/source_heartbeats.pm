package SNAG::Source::source_heartbeats;
    ...
    inline_states=>
    {
      ...
      send_alerts => sub
      {
        ...
        eval
        {
          ...
          my $check_connections = $dbh->selectall_arrayref("select * from activity() where usename like 'sysinfo_snags_%' and datname = 'sysinfo' and backend_start < ( now() - interval '10 minutes' ) and query_start > ( now() - interval '5 minutes')", { Slice => {} });
          ...
          if($check_connections && @$check_connections)
          {
            ...
            unless($locked)
            {
	      my $query = "SELECT * FROM server_heartbeats
	      WHERE server_heartbeats.seen < 
	      (now() - interval '180 seconds') AND 
	      source !~ 'aslc|snagc' AND server = 'master' 
	      AND server_heartbeats.seen > (now() - interval '7 days');";

              my $heartbeats = $dbh->selectall_arrayref($query, { Slice => {} });
              ...
              if($heartbeats)
              {
                ...
                {  
                  ...
                  if($ref->{server_seen} && (!$ref->{status} || $ref->{status} eq 'active'))
                  {
                    ...
                    {
                      $alert = "SNAGP/SNAGS heartbeat exceeds critical threshhold";
                      $event = "$ref->{source} heartbeat was at $last_seen (seconds old $seconds_old, critical threshhold $critical_thresh)";
                    }
                    elsif($seconds_old > $warning_thresh)
                    {
                      $alert = "SNAGP/SNAGS heartbeat exceeds warning threshhold";
                      $event = "$ref->{source} heartbeat was at $last_seen (seconds old $seconds_old, warning threshhold $warning_thresh)";
                    }
                    else
                    {
                      next;
