...
#################################
{
    ...
    POE::Component::Server::TCP->new(
        ...
        ClientConnected => sub {
            ...
            $kernel->call( 'logger' => 'log' => "ClientConnected: New connection from $ip ($host) timing out at "
                           . ( $now + 120 ) );
...
"SNAG::Server Error: got a disconnect from a server that wasn't connected, ip=$ip, host=$heap->{hostname}"
                );
            }
        ...
        ClientError => sub {
            my ( $kernel, $heap, $syscall_name, $error_number, $error_string, $extra ) =
              @_[ KERNEL, HEAP, ARG0, ARG1, ARG2 ];
            $kernel->call( 'logger' => 'log' =>
                           "SNAG::Server ClientError: $syscall_name:$error_number:$error_string:$heap->{remote_ip}" );
        },
        ...
        InlineStates => {
            ...
            handshake_timeout => sub {
                ...
                $server_data->{timeout}++;
                $kernel->call( 'logger' => 'log' =>
                               "Handshake Error: Client ($heap->{remote_ip}) sent nothing within time allowed ($now)" );
...
"update server_heartbeats set seen = ?, server_seen = now() where host = ? and server = ? and source = ?"
                            ...
                            while ( my ( $host, $ref ) = each %$heartbeat_spool ) {
                                while ( my ( $source, $seen ) = each %$ref ) {
                                    if ( $hb_update_sth->execute( $seen, $host, $alias, $source ) == 1 ) {
                                        ...
                                        $hb_insert_sth->execute( $host, $alias, $source, $seen )
                                          or die "Unable to update server_heartbeats: " . $sysinfo_dbh->errstr;
                                    }
                                }
                        ...
                        $kernel->call( 'logger' => 'log' =>
                                       "Could not update heartbeats, 'object' not connected to the sysinfo database!" );
                    }
                };
                if ($@) {
...
############################
{
    ...
    eval {
        ...
        if ( my $function = $parcel->{function} ) {
            ...
            if (     $function eq 'heartbeat'
                 and $server_data->{server_alias} =~ /^(sysinfo|dashboard|alerts|master|sysrrd)$/ )
            {
         ...
         #rv passes back valid data for functions now, if something goes wrong in $function the die needs to occur there
            if ( $rv && $rv == -1 ) {
                $kernel->post(
                          ...
                          (
                            'events',                   HOST_NAME,
                            'SNAG',                     "$server_data->{server_alias}",
                            'SNAG Server sending hold', "Server is sending $heap->{hostname} hold.  return value: $rv",
                            '', time2str( "%Y-%m-%d %T\n", time() )
                          )
                );
